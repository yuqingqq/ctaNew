"""Window × cadence grid: systematic validation of universe-update mechanism.

Tests every combination of:
  Windows:  60d, 90d, 120d, 180d, 240d, 365d
  Cadences: weekly (7d), biweekly (14d), monthly (30d), quarterly (90d),
            semi-annually (180d), never (static-calib only)

Plus a "static (calib only)" baseline that uses the calibration period (folds 0-4).
"""
...
OUT_DIR = REPO / "outputs/vBTC_window_cadence_grid"
...
WINDOWS_DAYS = [60, 90, 120, 180, 240, 365]
CADENCES_DAYS = [7, 14, 30, 90, 180]   # weekly, biweekly, monthly, quarterly, semi-annual
...
def build_schedule(all_pred_clean, prod_times, window_days, cadence_days):
    """Build universe-per-time map for given (window, cadence)."""
    bar_ms = 5 * 60 * 1000
    ...
    # Map each prod time to its update boundary
    schedule = {}
...
def main():
    ...
    # Static baseline: from calib folds 0-4
    ...
    print(f"\n=== Static baseline ===", flush=True)
    df_static = evaluate_with_dynamic_universe(test_data, static_universe)
    net_s = df_static["net_bps"].to_numpy()
    sh_s, lo_s, hi_s = block_bootstrap_ci(net_s, statistic=_sharpe, block_size=7, n_boot=2000)
    print(f"  static: Sharpe={sh_s:+.2f}  CI=[{lo_s:+.2f}, {hi_s:+.2f}]", flush=True)

    # Grid: window × cadence
    print(f"\n=== WINDOW × CADENCE GRID ===", flush=True)
    print(f"  {'window':<7} {'cadence':<10}  {'Sharpe':>7}  {'CI_lo':>7}  {'CI_hi':>7}",
          flush=True)
    results = [{"window_days": "static", "cadence_days": "never",
                  "sharpe": sh_s, "ci_lo": lo_s, "ci_hi": hi_s}]
    for window in WINDOWS_DAYS:
        for cadence in CADENCES_DAYS:
            t0 = time.time()
            schedule = build_schedule(all_pred_clean, prod_times_sampled, window, cadence)
            df_v = evaluate_with_dynamic_universe(test_data, schedule)
            if df_v.empty: continue
            net = df_v["net_bps"].to_numpy()
            sh, lo, hi = block_bootstrap_ci(net, statistic=_sharpe, block_size=7, n_boot=2000)
            results.append({"window_days": window, "cadence_days": cadence,
                              "sharpe": sh, "ci_lo": lo, "ci_hi": hi})
            print(f"  {window:>3}d    {cadence:>3}d-cadence  {sh:>+7.2f}  {lo:>+7.2f}  {hi:>+7.2f}  "
                  f"({time.time()-t0:.0f}s)", flush=True)

    # Pivot
    print(f"\n=== Sharpe matrix (rows=window, cols=update cadence) ===", flush=True)
    df_r = pd.DataFrame(results)
    df_r_dyn = df_r[df_r["window_days"] != "static"].copy()
    df_r_dyn["window_days"] = df_r_dyn["window_days"].astype(int)
    df_r_dyn["cadence_days"] = df_r_dyn["cadence_days"].astype(int)
    pivot = df_r_dyn.pivot(index="window_days", columns="cadence_days", values="sharpe")
    print(pivot.to_string(float_format=lambda x: f"{x:+.2f}"), flush=True)
    print(f"\n  Static (no updates): Sharpe={sh_s:+.2f}", flush=True)

    pd.DataFrame(results).to_csv(OUT_DIR / "window_cadence_grid.csv", index=False)
    pivot.to_csv(OUT_DIR / "sharpe_matrix.csv")

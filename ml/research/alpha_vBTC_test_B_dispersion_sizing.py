"""Test B: Continuous dispersion-based sizing.

Replace binary conv_gate (skip cycles with dispersion < 30th pctile) with
a continuous sizing factor: bigger position when dispersion is high (=
strong signal), smaller when low.

This is more graceful than the on/off gate. Should reduce variance while
preserving returns.

Configs:
  baseline (binary conv_gate)
  continuous_steep   (sigmoid steep, hard cutoff near 30 pctile)
  continuous_smooth  (sigmoid smooth, gradual sizing)
  linear_above_30    (no scale below 30 pctile, linear ramp above)
"""
...
OUT_DIR = REPO / "outputs/vBTC_test_B_dispersion"
...
COST_PER_LEG = 4.5
GATE_LOOKBACK = 252
GATE_PCTILE_BASELINE = 0.30   # binary gate threshold
PM_M = 2
PM_BAND = 1.0
MIN_OBS_PER_SYM = 100
TARGET_N = 15
...
def evaluate_with_sizing(test_df, universe_per_t, top_k=K, sizing_mode="binary",
                            cost_per_leg=COST_PER_LEG, sample_every=HORIZON):
    """sizing_mode:
      'binary'      — original conv_gate (skip if disp < 30th pctile)
      'continuous_steep'   — sigmoid: 0 below ~25 pct, 1 above ~35 pct
      'continuous_smooth'  — sigmoid: 0 below ~10 pct, 1 above ~50 pct
      'linear_above_30'    — 0 below 30 pct, linear ramp 0 to 1 above
    """
    df = test_df.copy()
    ...
    for t, g in df.groupby("open_time"):
        ...
        # Compute pctile of dispersion in trailing history
        size = 1.0
        if len(dispersion_history) >= 30:
            ...
            if sizing_mode == "binary":
                size = 1.0 if pctile >= GATE_PCTILE_BASELINE else 0.0
            elif sizing_mode == "continuous_steep":
                # sigmoid centered at 30 pct, steep slope
                size = 1.0 / (1.0 + np.exp(-(pctile - 0.30) * 20))
            elif sizing_mode == "continuous_smooth":
                # sigmoid centered at 30 pct, gentle slope
                size = 1.0 / (1.0 + np.exp(-(pctile - 0.30) * 6))
            elif sizing_mode == "linear_above_30":
                if pctile < 0.30:
                    size = 0.0
                else:
                    size = (pctile - 0.30) / 0.70   # 0 at 30 pct, 1 at 100 pct
        ...
        if size <= 0.001:
            # Hold-through (no size, just MtM if positions exist)
            if cur_long or cur_short:
                ...
                bars.append({"time": t, "net_bps": prev_size * (long_ret - short_ret) * 1e4, "size": 0.0})
            else:
                bars.append({"time": t, "net_bps": 0.0, "size": 0.0})
            continue
        ...
        # Cost scales with size and turnover
...
def main():
    ...
    print(f"\n=== Test B: dispersion-based sizing ===", flush=True)
    print(f"  {'config':<25}  {'Sharpe':>7}  {'std':>6}  {'max_DD':>7}  {'mean_size':>9}",
          flush=True)
    results = []
    for label in ["binary", "continuous_steep", "continuous_smooth", "linear_above_30"]:
        df_eval = evaluate_with_sizing(test_data, schedule_180_90, sizing_mode=label)
        ...
        results.append({"config": label, "sharpe": sh, "ci_lo": lo, "ci_hi": hi,
                          "std_bps": net.std(), "max_dd": max_dd, "mean_size": mean_size,
                          **{f"sh_f{f}": v for f, v in per_fold.items()}})
        print(f"  {label:<25}  {sh:>+7.2f}  {net.std():>6.1f}  {max_dd:>+7.0f}  "
              f"{mean_size:>9.2f}", flush=True)

    print(f"\n  Per-fold Sharpe:", flush=True)
    print(f"  {'config':<25}  " + " ".join(f"{'fold' + str(f):>8}" for f in PROD_FOLDS), flush=True)
    for r in results:
        cells = " ".join(f"{r.get(f'sh_f{f}', 0):+5.2f}" for f in PROD_FOLDS)
        print(f"  {r['config']:<25}  " + cells, flush=True)

    pd.DataFrame(results).to_csv(OUT_DIR / "test_B_results.csv", index=False)

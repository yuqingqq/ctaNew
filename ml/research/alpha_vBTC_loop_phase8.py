"""Loop Phase 8: N (universe size) sweep on winner_21 with K=4.

Adding 5 new features may shift optimal N too. Test N ∈ {10, 12, 15, 20, 25}
on K=4 with the winning 21-feature set.
"""
...
OUT_DIR = REPO / "outputs/vBTC_loop_phase8"
...
def main():
    ...
    print(f"  Winner_21 ({len(feat_set)} features), 5 seeds, K={K}", flush=True)

    print(f"\n=== Training 5-seed ensemble per fold (full universe) ===", flush=True)
    fold_data = {}
    ...
    print(f"\n=== N SWEEP (K={K}) ===", flush=True)
    print(f"  {'N':>3} {'avg_IC':>8}  {'n_cyc':>5}  {'mean':>6}  {'cost':>5}  "
          f"{'Sharpe':>7}  {'CI_lo':>7}  {'CI_hi':>7}", flush=True)
    summary = []
    for n in N_VALUES:
        if 2 * K + 1 > n: continue
        universe = set(df_ic_calib["symbol"].head(n).tolist())
        avg_ic = df_ic_calib.iloc[:n]["ic"].mean()
        cycles = []
        for fid, (test_df, pred) in fold_data.items():
            keep_mask = test_df["symbol"].isin(universe).to_numpy()
            test_f = test_df[keep_mask].copy()
        ...
        summary.append({"N": n, "avg_ic": avg_ic, "n_cycles": len(net),
                          "mean": net.mean(), "cost": df_v["cost"].mean(),
                          "sharpe": sh, "ci_lo": lo, "ci_hi": hi,
                          **{f"sh_f{f}": v for f, v in per_fold_sh.items()}})
        print(f"  {n:>3} {avg_ic:>+8.4f}  {len(net):>5}  {net.mean():>+6.2f}  "
              f"{df_v['cost'].mean():>+5.2f}  {sh:>+7.2f}  {lo:>+7.2f}  {hi:>+7.2f}", flush=True)

    print(f"\n  Per-fold Sharpe:", flush=True)
    print(f"  {'N':>3}  " + " ".join(f"{'fold' + str(f):>9}" for f in PROD_FOLDS), flush=True)
    for r in summary:
        cells = " ".join(f"{r.get(f'sh_f{f}', 0):+5.2f}" for f in PROD_FOLDS)
        print(f"  {r['N']:>3}  " + cells, flush=True)

    pd.DataFrame(summary).to_csv(OUT_DIR / "phase8_n_sweep.csv", index=False)

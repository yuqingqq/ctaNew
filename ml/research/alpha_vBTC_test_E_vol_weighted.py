"""Test E: per-name volatility-targeted position sizing.

Instead of equal weight 1/K per name, weight each picked name by 1/σ_name.

This caps high-vol names (VVV ~2.4% σ) and boosts low-vol majors (BTC ~0.5%).
Should reduce drawdowns since worst losses come from VVV's extreme moves.
"""
...
OUT_DIR = REPO / "outputs/vBTC_test_E_vol_weighted"
...
def evaluate_vol_weighted(test_df, universe, top_k=K, weight_mode="equal",
                              cost_per_leg=COST_PER_LEG, sample_every=HORIZON):
    """weight_mode:
      'equal'         — current 1/K per pick (baseline)
      'inv_vol'       — weight by 1/atr_pct (per-cycle)
      'inv_vol_cap'   — inv_vol but capped at 2× equal weight
    """
    df = test_df.copy()
    ...
    for t, g in df.groupby("open_time"):
        ...
        atr_arr = g_u["atr_pct"].to_numpy() if "atr_pct" in g_u.columns else np.ones(len(g_u))
        ...
        # Compute weights
        def get_weights(side_g, mode):
            atr = side_g["atr_pct"].to_numpy()
            atr = np.where(np.isnan(atr) | (atr <= 0), atr.mean() if (atr > 0).any() else 0.01, atr)
            if mode == "equal":
                return np.ones(len(side_g)) / len(side_g)
            elif mode == "inv_vol":
                w = 1.0 / atr
                return w / w.sum()
            elif mode == "inv_vol_cap":
                w = 1.0 / atr
                w_norm = w / w.sum()
                cap = 2.0 / len(side_g)
        ...
        w_long = get_weights(long_g, weight_mode)
        w_short = get_weights(short_g, weight_mode)
        long_ret = float((long_g["return_pct"].to_numpy() * w_long).sum())
        short_ret = float((short_g["return_pct"].to_numpy() * w_short).sum())
...
def main():
    ...
    for fid, (td, p) in fold_data.items():
        df = td[["symbol", "open_time", "alpha_A", "return_pct", "atr_pct"]].copy()
    ...
    test_data = prod_pred[["symbol", "open_time", "pred", "return_pct", "alpha_A", "atr_pct"]].copy()

    print(f"\n=== Test E: per-name vol-targeting ===", flush=True)
    print(f"  {'config':<22}  {'Sharpe':>7}  {'std':>6}  {'max_DD':>7}  {'mean':>6}", flush=True)
    results = []
    for label in ["equal", "inv_vol", "inv_vol_cap"]:
        df_eval = evaluate_vol_weighted(test_data, static_universe, top_k=K, weight_mode=label)
        ...
        print(f"  {label:<22}  {sh:>+7.2f}  {net.std():>6.1f}  {max_dd:>+7.0f}  {net.mean():>+6.2f}",
              flush=True)

    print(f"\n  Per-fold Sharpe:", flush=True)
    print(f"  {'config':<22}  " + " ".join(f"{'fold' + str(f):>8}" for f in PROD_FOLDS),
          flush=True)
    for r in results:
        cells = " ".join(f"{r.get(f'sh_f{f}', 0):+5.2f}" for f in PROD_FOLDS)
        print(f"  {r['config']:<22}  " + cells, flush=True)

    pd.DataFrame(results).to_csv(OUT_DIR / "test_E_results.csv", index=False)

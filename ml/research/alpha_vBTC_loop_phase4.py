"""Loop Phase 4: stack the winning feature additions from Phase 3.

Phase 3 winners:
  +xs_aggs       — Sharpe +3.47 (best, but 2 negative folds)
  +cross_btc     — Sharpe +3.22 (all 5 folds positive)
  +more_funding  — Sharpe +3.00 (good)

Question: do they ADD (stack benefits) or SUBSTITUTE (overlap)?

Configs (all built from dedup_16_fund base):
  baseline_16
  +xs_aggs                     (best single addition)
  +cross_btc                   (most consistent)
  +xs_aggs +cross_btc          (top 2 stacked)
  +xs_aggs +more_funding       (top + funding)
  +cross_btc +more_funding     (consistent + funding)
  +ALL_THREE                   (everything)
"""
...
OUT_DIR = REPO / "outputs/vBTC_loop_phase4"
...
def main():
    ...
    configs = [
        ...
        ("+xs",                    DEDUP_16_FUND + ADD_XS_AGGS),
        ("+btc",                   DEDUP_16_FUND + ADD_CROSS_BTC),
        ("+xs+btc",                DEDUP_16_FUND + ADD_XS_AGGS + ADD_CROSS_BTC),
        ("+xs+fund",               DEDUP_16_FUND + ADD_XS_AGGS + ADD_MORE_FUNDING),
    ...
    print(f"\n{'=' * 100}", flush=True)
    print(f"PHASE 4 — STACKING WINNING FEATURES (from Phase 3)", flush=True)
    ...
    pd.DataFrame(results).to_csv(OUT_DIR / "phase4_stacked.csv", index=False)

"""Final validation: 10-seed ensemble of the FULL winning architecture.

Architecture under test:
  Universe filter: 180-day window × 90-day quarterly refresh
  Features: dedup_16 + cross_btc + more_funding (21 features)
  Model: 10-seed LGBM ensemble
  Strategy: K=4 long, K=4 short, conv_gate + PM_M2_b1
  Universe size: top-15 by IC at each refresh

Compares to:
  static (calibration once, no updates)
  best_grid (180d × 90d, 5-seed)

If 10-seed of the winner ≈ 5-seed result, the +4.09 was real.
If 10-seed drops materially, the result was 5-seed overstatement.
"""
...
OUT_DIR = REPO / "outputs/vBTC_final_validation"
...
def evaluate_with_dynamic_universe(test_df, universe_per_t, top_k=K, cost_per_leg=COST_PER_LEG,
    ...
    for t, g in df.groupby("open_time"):
        ...
        if len(g_u) < 2 * top_k + 1:
            bars.append({"time": t, "fold_t": str(t)[:7], "net_bps": 0.0, "skipped": 1})
            continue
...
def build_schedule(all_pred_clean, prod_times, window_days, cadence_days):
    ...
    return schedule, boundary_to_universe


def run_full(panel, feat_set, seeds, label, folds_all):
    """Train ensemble per fold, return per-fold (test_df, pred)."""
    print(f"\n=== Training {label} ({len(seeds)} seeds) ===", flush=True)
    fold_data = {}
    for fid in ALL_FOLDS:
        if fid >= len(folds_all): continue
        t0 = time.time()
        test_df, pred = train_fold(panel, folds_all[fid], feat_set, seeds)
        if test_df is None: continue
        fold_data[fid] = (test_df, pred)
...
def main():
    ...
    # Train BOTH 5-seed and 10-seed ensembles for direct comparison
    fold_data_10 = run_full(panel, feat_set, SEEDS_10, "10-seed", folds_all)

    all_preds_10 = []
    for fid, (td, p) in fold_data_10.items():
        df = td[["symbol", "open_time", "alpha_A", "return_pct"]].copy()
        df["pred"] = p; df["fold"] = fid
        all_preds_10.append(df)
    all_pred_df_10 = pd.concat(all_preds_10, ignore_index=True).sort_values(["open_time", "symbol"])
    ts = all_pred_df_10["open_time"]
    if hasattr(ts.dtype, "tz") and ts.dtype.tz is not None:
        ts_naive = ts.dt.tz_convert("UTC").dt.tz_localize(None)
    else:
        ts_naive = ts
    all_pred_df_10["t_int"] = ts_naive.astype("datetime64[ms]").astype("int64").to_numpy()
    all_pred_clean_10 = all_pred_df_10.dropna(subset=["alpha_A"])

    prod_pred_10 = all_pred_df_10[all_pred_df_10["fold"].isin(PROD_FOLDS)].copy()
    prod_times = sorted(prod_pred_10["open_time"].unique())
    prod_times_sampled = prod_times[::HORIZON]
    test_data_10 = prod_pred_10[["symbol", "open_time", "pred", "return_pct", "alpha_A"]].copy()

    # Static universe (calib folds 0-4 IC)
    calib = all_pred_df_10[all_pred_df_10["fold"].isin([0, 1, 2, 3, 4])].dropna(subset=["alpha_A"])
    ...
    # 180d × 90d schedule
    schedule_180_90, boundary_universes = build_schedule(all_pred_clean_10, prod_times_sampled, 180, 90)

    print(f"\n=== 10-SEED VALIDATION ===", flush=True)
    print(f"  Static universe: {sorted(static_universe)}", flush=True)
    print(f"\n  180d × 90d universe schedule:", flush=True)
    for b, u in sorted(boundary_universes.items()):
        b_dt = pd.Timestamp(b, unit="ms")
        print(f"    boundary {b_dt.date()}: {sorted(u)}", flush=True)

    print(f"\n=== Strategy Sharpe (10-seed ensemble, K={K}) ===", flush=True)
    results = []
    for label, universe in [("static", static_universe),
                              ("180d_x_90d", schedule_180_90)]:
        df_v = evaluate_with_dynamic_universe(test_data_10, universe)
        if df_v.empty: continue
        net = df_v["net_bps"].to_numpy()
        sh, lo, hi = block_bootstrap_ci(net, statistic=_sharpe, block_size=7, n_boot=2000)
        n_skipped = int(df_v["skipped"].sum())
        # Per-fold breakdown by mapping time → fold
        per_fold = {}
        for fid in PROD_FOLDS:
            fold_tests = prod_pred_10[prod_pred_10["fold"] == fid]["open_time"]
            fold_t_set = set(fold_tests.unique())
            mask = df_v["time"].isin(fold_t_set)
            n_f = df_v.loc[mask, "net_bps"].to_numpy()
            if len(n_f) >= 3:
                per_fold[fid] = _sharpe(n_f)
        results.append({"label": label, "n": len(net), "skipped": n_skipped,
                          "mean": net.mean(), "sharpe": sh, "ci_lo": lo, "ci_hi": hi,
                          "per_fold": per_fold})
        print(f"  {label:<12}: n={len(net)}  skipped={n_skipped}  mean={net.mean():+.2f}  "
    ...
    for r in results:
        ...
        print(f"  {r['label']:<12}  " + cells, flush=True)

    pd.DataFrame([{**{k: r[k] for k in r if k != "per_fold"}} for r in results]
                  ).to_csv(OUT_DIR / "final_validation.csv", index=False)

"""Loop Phase 3: add candidate new features to dedup_16_fund base.

Tests 5 categories of additions to find which type of new feature helps most.
Then a combined config with all helpful additions.

Configs:
  baseline_16        — dedup_16_fund (Sharpe +2.52, current best)
  +4h_flow          — adds 5 4h microstructure features
  +more_funding     — adds 2 more funding features
  +cross_btc        — adds 3 BTC-cross-asset features
  +idio_chars       — adds 3 idiosyncratic shape features
  +xs_aggs          — adds 2 cross-sectional dispersion features
"""
...
OUT_DIR = REPO / "outputs/vBTC_loop_phase3"
...
ALL_DROPS = DEDUP_DROPS_TIER1 + DEDUP_DROPS_TIER2
FUNDING_LEAN = ["funding_rate", "funding_rate_z_7d"]
DEDUP_16_FUND = [f for f in V6_CLEAN_28 if f not in ALL_DROPS] + FUNDING_LEAN

# New feature categories to test
ADD_4H_FLOW       = ["avg_trade_size_4h", "aggr_ratio_4h", "tfi_4h",
                       "buy_count_4h", "signed_volume_4h"]
...
ADD_IDIO_CHARS    = ["idio_skew_1d", "idio_kurt_1d", "name_factor_loading_1d"]
...
def main():
    ...
    configs = [
        ...
        ("+4h_flow",            DEDUP_16_FUND + ADD_4H_FLOW),
        ("+more_funding",       DEDUP_16_FUND + ADD_MORE_FUNDING),
        ("+cross_btc",          DEDUP_16_FUND + ADD_CROSS_BTC),
        ("+idio_chars",         DEDUP_16_FUND + ADD_IDIO_CHARS),
        ("+xs_aggs",            DEDUP_16_FUND + ADD_XS_AGGS),
    ]
    results = []
    for label, feats in configs:
        # Skip features missing from panel
        feats = [f for f in feats if f in panel.columns]
        r = run_config(panel, folds, feats, universe, label)
        if r: results.append(r)

    print(f"\n{'=' * 100}", flush=True)
    print(f"PHASE 3 — NEW FEATURE ADDITIONS (base = dedup_16_fund @ +2.52)", flush=True)
    ...
    pd.DataFrame(results).to_csv(OUT_DIR / "phase3_addition_compare.csv", index=False)

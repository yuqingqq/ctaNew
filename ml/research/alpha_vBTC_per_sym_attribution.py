"""Per-symbol attribution on the final architecture (F21+K=3+180x90, 10-seed).

For each symbol in the trade universe across production folds:
  - Total PnL contribution
  - Long PnL / Short PnL split
  - Number of cycles in long / short
  - Per-symbol IC vs realized alpha

Production-relevant: tells which names actually drive returns and
which are passengers. If one name dominates, that's concentration risk.
"""
...
OUT_DIR = REPO / "outputs/vBTC_per_sym_attr"
...
def evaluate_with_attribution(test_df, universe_per_t, top_k=K, sample_every=HORIZON):
    """Like evaluate_with_dynamic_universe but track per-symbol PnL contributions."""
    df = test_df.copy()
    ...
    bars = []
    contributions = []   # per-cycle per-symbol contribution
    ...
    for t, g in df.groupby("open_time"):
        ...
        if skip:
            if cur_long or cur_short:
                ...
                # Hold-through: each symbol contributes its return
                for s in cur_long:
                    sub = long_g[long_g["symbol"] == s]
                    if not sub.empty:
                        contributions.append({"time": t, "symbol": s, "side": "long_hold",
                                              "ret_bps": float(sub["return_pct"].iloc[0]) * 1e4 / len(cur_long)})
                for s in cur_short:
                    sub = short_g[short_g["symbol"] == s]
                    if not sub.empty:
                        contributions.append({"time": t, "symbol": s, "side": "short_hold",
                                              "ret_bps": -float(sub["return_pct"].iloc[0]) * 1e4 / len(cur_short)})
            else:
                bars.append({"time": t, "net_bps": 0.0, "skipped": 1})
            continue
...
def main():
    ...
    print(f"\n=== Evaluating final architecture (F21 K=3 180×90, 10-seed) ===", flush=True)
    df_eval, df_contrib = evaluate_with_attribution(test_data, schedule_180_90, top_k=K)
    net = df_eval["net_bps"].to_numpy()
    print(f"  n_cycles={len(net)}  Sharpe={net.mean()/net.std()*np.sqrt(2190):+.2f}  "
          f"mean={net.mean():+.2f}", flush=True)

    # Per-symbol attribution
    print(f"\n=== Per-symbol PnL contribution ===", flush=True)
    sym_attr = df_contrib.groupby("symbol")["ret_bps"].agg(["sum", "count", "mean"])
    sym_attr.columns = ["total_bps", "n_picks", "avg_bps_per_pick"]

    # Long vs short breakdown
    long_attr = df_contrib[df_contrib["side"].str.startswith("long")].groupby("symbol")["ret_bps"].agg(["sum", "count"])
    short_attr = df_contrib[df_contrib["side"].str.startswith("short")].groupby("symbol")["ret_bps"].agg(["sum", "count"])
    long_attr.columns = ["long_pnl_bps", "long_picks"]
    short_attr.columns = ["short_pnl_bps", "short_picks"]
    sym_attr = sym_attr.join(long_attr, how="left").join(short_attr, how="left").fillna(0)
    sym_attr = sym_attr.sort_values("total_bps", ascending=False)

    print(f"  {'symbol':<14} {'total_bps':>10}  {'n_picks':>7}  {'long_picks':>10}  "
          f"{'short_picks':>11}  {'long_pnl':>9}  {'short_pnl':>10}", flush=True)
    for s, row in sym_attr.iterrows():
        print(f"  {s:<14} {row['total_bps']:>+10.0f}  {int(row['n_picks']):>7}  "
              f"{int(row['long_picks']):>10}  {int(row['short_picks']):>11}  "
              f"{row['long_pnl_bps']:>+9.0f}  {row['short_pnl_bps']:>+10.0f}", flush=True)

    print(f"\n  Concentration:", flush=True)
    total = sym_attr["total_bps"].sum()
    top3_total = sym_attr["total_bps"].head(3).sum()
    bot3_total = sym_attr["total_bps"].tail(3).sum()
    print(f"    Total PnL across all picks: {total:+.0f} bps", flush=True)
    print(f"    Top 3 names contribute: {top3_total:+.0f} bps ({top3_total/total*100:.1f}% of total)",
          flush=True)
    print(f"    Bottom 3 names contribute: {bot3_total:+.0f} bps", flush=True)

    sym_attr.to_csv(OUT_DIR / "per_symbol_attribution.csv")

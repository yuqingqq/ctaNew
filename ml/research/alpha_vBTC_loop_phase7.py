"""Loop Phase 7: 10-seed validation of K=4 with winner_21 features.

5-seed Phase 6 found K=4 @ Sharpe +5.19 [+2.02, +8.45], all 5 folds positive.
This is the best result of the entire investigation. Confirming with 10 seeds.
...
OUT_DIR = REPO / "outputs/vBTC_loop_phase7"
...
def main():
    ...
    print(f"  Winner_21 ({len(feat_set)} features), {len(SEEDS)} seeds", flush=True)

    print(f"\n=== Training 10-seed ensemble per fold ===", flush=True)
    fold_data = {}
    ...
    pd.DataFrame(summary).to_csv(OUT_DIR / "phase7_validation.csv", index=False)

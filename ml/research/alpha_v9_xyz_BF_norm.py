"""Audit completeness: normalize B (PEAD) and F (sector) features.

Path B/F: do the B and F features benefit from xs-rank normalization?

Variants:
  baseline           = 18 raw v7 features
  B_xs_add           = baseline + xs-rank versions of 4 B features (22)
  F_xs_add           = baseline + xs-rank versions of 3 F features (21)
  ABF_xs_add         = baseline + xs-rank versions of all A+B+F (35)

7-fold walk-forward, 3 seeds × 1 horizon. Same discipline gates.

Hypothesis from the audit: B and F normalization will follow the same
wash-or-fail pattern as A normalization. PEAD has discrete semantics
(days_since_earn, surprise %) that may not benefit from rank; sector is
already aggregated.

Usage:
    python -m ml.research.alpha_v9_xyz_BF_norm
"""
...
def main():
    ...
    variants = {
        ...
        "B_xs_add (22)": base_feats + feats_B_xs,
        "F_xs_add (21)": base_feats + feats_F_xs,
        "ABF_xs_add (35)": base_feats + feats_A_xs + feats_B_xs + feats_F_xs,
    }
    ...
    log.info("\n=== Aggregated 7-fold OOS (Tier A+B, K=4 M=1, gate, 0.8 bps) ===")

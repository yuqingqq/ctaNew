"""Loop Phase 2: batched dedup of correlated features.

Tests progressively more aggressive removal of correlated/weak features
from dedup_23_fund (current best Sharpe +1.52 at 10 seeds).

Configs:
  dedup_23_fund (baseline)        — current best
  dedup_18_fund (drop 5 redundant) — atr_pct_xs_rank, dom_z_7d_vs_bk,
                                      obv_z_1d_xs_rank, obv_signal,
                                      price_volume_corr_10
  dedup_16_fund (drop 7)          — Tier 1 + hour_cos, hour_sin
  dedup_14_fund (drop 9)          — Tier 2 + atr_pct, vwap_slope_96
"""
...
OUT_DIR = REPO / "outputs/vBTC_loop_phase2"
...
# Tier 1: redundant pairs (drop lower-gain member of high-correlation pair)
TIER1_DROPS = ["atr_pct_xs_rank", "dom_z_7d_vs_bk", "obv_z_1d_xs_rank",
               "obv_signal", "price_volume_corr_10"]
# Tier 2: + very-weak time features
TIER2_DROPS = TIER1_DROPS + ["hour_cos", "hour_sin"]
# Tier 3: + more dubious low-gain features
TIER3_DROPS = TIER2_DROPS + ["atr_pct", "vwap_slope_96"]

DEDUP_18_FUND = [f for f in DEDUP_21 if f not in TIER1_DROPS] + FUNDING_LEAN
DEDUP_16_FUND = [f for f in DEDUP_21 if f not in TIER2_DROPS] + FUNDING_LEAN
DEDUP_14_FUND = [f for f in DEDUP_21 if f not in TIER3_DROPS] + FUNDING_LEAN
...
def main():
    ...
    configs = [
        ("dedup_23_fund",  DEDUP_23_FUND),
        ("dedup_18_fund",  DEDUP_18_FUND),
        ("dedup_16_fund",  DEDUP_16_FUND),
        ("dedup_14_fund",  DEDUP_14_FUND),
    ]
    results = []
    ...
    print(f"\n{'=' * 100}", flush=True)
    print(f"PHASE 2 — BATCHED DEDUP COMPARISON (N={TOP_N}, K={K}, 5-seed)", flush=True)
    ...
    pd.DataFrame(results).to_csv(OUT_DIR / "phase2_batched_dedup.csv", index=False)

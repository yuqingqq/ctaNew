"""Compare baseline_28 vs v6_clean + funding features.

Configurations:
  baseline_28        — current v6_clean 28 features (Sharpe +1.97)
  baseline_30_lean   — v6_clean + 2 funding features (funding_rate, funding_rate_z_7d)
  baseline_31_full   — v6_clean + 3 funding features (+ funding_streak_pos)
  baseline_33_all    — v6_clean + all 5 funding features
"""
...
OUT_DIR = REPO / "outputs/vBTC_funding_compare"
...
V7_LEAN_FUNDING = ["funding_rate", "funding_rate_z_7d"]
V7_FULL_FUNDING = ["funding_rate", "funding_rate_z_7d", "funding_streak_pos"]
V7_ALL_FUNDING = ["funding_rate", "funding_rate_z_7d", "funding_rate_1d_change",
...
def main():
    ...
    configs = [
        ...
        ("v7_lean_30",        V6_CLEAN_28 + V7_LEAN_FUNDING),
        ("v7_full_31",        V6_CLEAN_28 + V7_FULL_FUNDING),
        ("v7_all_33",         V6_CLEAN_28 + V7_ALL_FUNDING),
    ]
    results = []
    ...
    print(f"\n{'=' * 100}", flush=True)
    print(f"FUNDING FEATURE COMPARISON (N={TOP_N}, K={K}, 5-seed ensemble)", flush=True)
    print(f"{'=' * 100}", flush=True)
    print(f"  {'config':<14} {'n_feat':>6}  {'avg_IC':>8}  {'Sharpe':>7}  "
    ...
    pd.DataFrame(results).to_csv(OUT_DIR / "funding_comparison.csv", index=False)

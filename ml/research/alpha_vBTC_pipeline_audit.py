...
def main():
  ...
  In the production pipeline:
    ...
    3. At universe boundary b, IC is computed using:
         past = all_pred[
             (t_int >= b - 180d) AND (t_int < b) AND (exit_time <= b)
         ]
  ...
  Answer: NO, because:
    ...
    - So predictions used in IC at boundary b were all generated by models trained
      strictly before time t < b.
    - The 2026-05-11 fix additionally requires exit_time <= b, so labels whose
      4h forward realization crosses the boundary are not used in that boundary's IC.
""", flush=True)
  ...
  By construction (see alpha_vBTC_final_simulation.py evaluate_flat_real):
    ...
    - DD overlay (apply_dd_tier_aggressive): uses pre-cycle sized cumulative PnL
...
""", flush=True)
  ...
  apply_dd_tier_aggressive(net):
    for i in range(n):
        dd_pct = (peak_before_i - cum_before_i) / peak_before_i
        sizes[i] = ... based on dd_pct ...

  At row i:
    - cum_before_i = sum of sized net_bps[0..i-1]
    - peak_before_i = past peak before cycle i
    - dd_pct uses only information known before entering cycle i
    - size[i] is then applied to cycle i's PnL

  2026-05-11 correction: the previous implementation computed drawdown from
  cum[i], which included the current cycle's PnL, then applied size[i] back to
  that same cycle. That was look-ahead. The final simulation now computes size
  from pre-cycle cumulative PnL and updates cumulative PnL after the sized cycle.

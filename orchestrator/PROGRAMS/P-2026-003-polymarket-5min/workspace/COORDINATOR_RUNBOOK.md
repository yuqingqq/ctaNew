...
- **Tip:** see `git log`. Next register entry after R-746: **R-747**.
...
**A refresh under a running unit (learned at this reset):** the reload brief told DE to `wt_refresh` its worktree while `de115day06_2` (WorkingDirectory = that worktree) was still running; the files under the running process moved from 04d3eb3 to the tip. DE assessed it harmless (the runner imports nothing further from the tree after start), but the rule for next time is: NEVER refresh a worktree named as a running unit's `--working-directory` until the unit has exited — reload the seat, let it read, and refresh after the exit.

"""Phase AH-native v3: simplified protocol (no SS filter, no PM persistence).

v2 diverged from Phase M due to picks_history accumulation drift in SS filter.
v3 removes SS filter and PM persistence entirely. This gives a clean
basket protocol where h=48/96/288 comparison is apples-to-apples and not
contaminated by filter-state divergence.

Logic per cycle:
  1. Get N=15 universe (rolling-IC)
  2. conv_gate (skip if pred_disp < 30th pctile of trailing)
  3. If trade: pick top-K longs, bot-K shorts by pred (no filter_refill)
  4. PM=1 (no persistence — each cycle picks fresh basket)
  5. Compute realized return at h_bars from fwd_rets
  6. churn-based cost
"""
...
OUT = REPO / "outputs/vBTC_native_horizon_v3"
...
def evaluate_simple(apd, universe, fwd_rets, h_bars, gate_lookback,
                     placebo_seed=None):
    """No SS filter, no PM. Just conv_gate + top-K selection."""
    ...
    for t in times:
        ...
        if placebo_seed is not None:
            ...
            nl = set(sym_arr[shuffled[:K]].tolist())
            ns = set(sym_arr[shuffled[K:2*K]].tolist())
        else:
            nl = set(sym_arr[idx_t].tolist())
            ns = set(sym_arr[idx_b].tolist())
        # Validate returns
        nl_valid = {s for s in nl if s in rets_at_t.index and not pd.isna(rets_at_t[s])}
        ns_valid = {s for s in ns if s in rets_at_t.index and not pd.isna(rets_at_t[s])}
        ...
        lr = [rets_at_t[s] for s in nl]
        sr = [rets_at_t[s] for s in ns]
...
def main():
    print("=== Phase AH-native v3: simplified protocol (no SS filter, no PM) ===\n",
          flush=True)
    ...
    print(f"  computing forward returns h ∈ {{48, 96, 144, 288}}...", flush=True)
    fwd_rets = {h: compute_forward_returns(close_wide, h) for h in [48, 96, 144, 288]}

    target_t_all = sorted(apd[apd["fold"].isin(OOS_FOLDS)]["open_time"].unique())

    variants = [48, 96, 144, 288]
    ...
    best = cands[best_h]
    print(f"\n  Baseline h=48 (no SS): Sharpe={base['sharpe']:+.2f}, "
          f"folds={base['n_folds_positive']}/9", flush=True)
    print(f"  Best h={best_h}: Sharpe={best['sharpe']:+.2f}, "
    ...
    if lift > 0.30:
        ...
        for seed in range(N_PLACEBO_SEEDS):
            df_p = evaluate_simple(apd, best["universe"], fwd_rets[best_h], best_h,
                                       int(best["cpy"]), placebo_seed=seed)
    ...
    print(f"\n=== Phase AH-native v3 verdict ===", flush=True)
    pass_lift = lift >= 0.30

"""Build STRICT-PIT BTC-frame + V2 features for 111-input panel.

Differences vs build_btc_features_111.py:
  1. beta_btc_pit: .shift(49) (was .shift(1)) — matches 01_build_target.py for 51-panel
     ensures rolling regression window is FULLY behind the 48-bar forward target
  2. All rolling-ending-at-current features explicitly .shift(1) so window end is
     bar t-1, not bar t. Under "position opens at open_time t" execution, this
     ensures features at row t only use data from bars [..., t-1] (close values
     known at open_time t):
       - corr_to_btc_1d
       - idio_vol_to_btc_1h, idio_vol_to_btc_1d
       - dom_btc_z_1d (uses rolling mean/std)
       - obv_z_1d
       - dom_btc_change_288b (diff endpoint)
       - corr_to_btc_change_3d inherits from shifted corr_to_btc_1d
       - beta_to_btc_change_5d inherits from shifted beta_btc_pit (already shift(49))
  3. return_8h, vol_zscore_4h_over_7d already shifted per Step 29 convention (.shift(1)
     and .shift(49) respectively)
  4. BTCUSDT excluded from output panel

Saves: outputs/vBTC_features_btc_only_111_strict_pit/panel_btc_only_111.parquet
"""
...
OUT_DIR = REPO / "outputs/vBTC_features_btc_only_111_strict_pit"
...
def main():
    print("=== Build STRICT-PIT BTC-frame + V2 features for 111-input panel ===\n",
          flush=True)
    ...
    print("\n--- Step 2: per-symbol feature engineering (STRICT PIT) ---", flush=True)
    t_start = time.time()
    ...
    for i, sym in enumerate(panel_syms):
        ...
        # Symmetric low-side distance-from-extreme feature.
    ...
    # Step 3: Merge with 111-panel (V2-essential cols only)
    ...
    # Step 4: bars_since_*_xs_rank — cross-sectional rank
    print("\n--- Step 4: bars_since_*_xs_rank (cross-sectional) ---", flush=True)
    ...
    print("\n--- Step 5: build alpha_beta target (STRICT PIT) ---", flush=True)

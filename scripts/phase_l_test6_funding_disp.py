"""Phase L / Test 6: funding_dispersion as gate signal (orthogonal-data probe).

Only in-scope orthogonal candidate from the user's Test 6 list (OI was
already rejected in v9 at Δsh=-3.05; L2 depth & cross-exchange basis need
paid data; liquidation flow not in current panel).

Funding_dispersion at time t = cross-symbol std(funding_rate) within the
rolling N=15 universe. Gate semantics: skip cycle if score < 30th pctile of
trailing 252.

Variants:
  V0 production (pred_disp)
  V1 funding_dispersion gate
  V2 pred_disp × funding_dispersion (composite)

Matched skip-placebo on best. Pass: beats production AND p95 AND ≥6/9 folds.
"""
...
OUT = REPO / "outputs/vBTC_funding_disp"
...
def evaluate(apd, universe, gate_name, panel_by_time, rng_seed=0,
    ...
    for t in times:
        ...
        if len(g_u) < 2 * K + 1:
            rows.append({"time": t, "fold": fold_lookup.get(t, 0), "score": np.nan,
                          "skipped": 1, "net_bps": 0, "n_long": 0, "n_short": 0})
            continue
        ...
        # Funding dispersion within universe at time t
        fund_disp = 0.0
        p_at_t = panel_by_time.get(t)
        if p_at_t is not None:
            p_u = p_at_t[p_at_t["symbol"].isin(u)]
            if len(p_u) > 0:
                v = p_u["funding_rate"].std()
                if not pd.isna(v): fund_disp = float(v)

        if gate_name == "V0_production":
            score = pred_disp
        elif gate_name == "V1_funding_disp":
            score = fund_disp
        elif gate_name == "V2_disp_x_fund_disp":
            score = pred_disp * fund_disp
        else:
            score = pred_disp
        ...
        if skip:
            if not is_flat and (cur_long or cur_short):
                rows.append({"time": t, "fold": fold_lookup.get(t, 0), "score": score,
                              "skipped": 1, "net_bps": -2 * COST_PER_LEG,
                              "n_long": 0, "n_short": 0})
        ...
        if not nl or not ns:
            if not is_flat and (cur_long or cur_short):
                ...
                rows.append({"time": t, "fold": fold_lookup.get(t, 0), "score": score,
                              "skipped": 0, "net_bps": 0, "n_long": 0, "n_short": 0})
            continue
...
def main():
    print("=== Phase L / Test 6: funding_dispersion gate ===\n", flush=True)
    apd = pd.read_parquet(APD_PATH)
    apd["open_time"] = pd.to_datetime(apd["open_time"], utc=True)
    apd["exit_time"] = pd.to_datetime(apd["exit_time"], utc=True)

    panel = pd.read_parquet(PANEL_PATH,
                              columns=["open_time", "symbol", "funding_rate"])
    ...
    variants = ["V0_production", "V1_funding_disp", "V2_disp_x_fund_disp"]
    print(f"\n  {'variant':<24}  {'Sharpe':>7}  {'CI':>17}  {'skip%':>6}  "
    ...
    for v in variants:
        ...
        print(f"  {v:<24}  {sh:>+7.2f}  [{lo:>+5.2f},{hi:>+5.2f}]  "
    ...
    print(f"\n  Production: Sharpe={prod['sharpe']:+.2f}, folds={prod['n_folds_positive']}/9",
          flush=True)
    print(f"  Best ({best['variant']}): Sharpe={best['sharpe']:+.2f}, "
          f"folds={best['n_folds_positive']}/9", flush=True)
    lift = best["sharpe"] - prod["sharpe"]
    print(f"  Lift: {lift:+.2f}", flush=True)

    # Matched skip-placebo
    print(f"\n--- Matched skip-placebo ({N_PLACEBO_SEEDS} seeds at "
          f"{best['skip_rate']*100:.1f}%) ---", flush=True)
    t0 = time.time()
    placebo_sh = []
    for seed in range(N_PLACEBO_SEEDS):
        df_p = evaluate(apd, universe, "V0_production", panel_by_time,
    ...
    print(f"  Best variant ranks p{rank:.0f}  "

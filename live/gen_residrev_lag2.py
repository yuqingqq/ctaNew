...
-> live/state/convexity/hl_residrev_lag2/{fullflow_hl60,v0full_hl60}.parquet
"""
...
OUT = REPO/"live/state/convexity/hl_residrev_lag2"; OUT.mkdir(parents=True, exist_ok=True)
...
PAN["resid_rev_2"] = -a.transform(lambda s: s.shift(2).rolling(2).sum())   # 8h
PAN["resid_rev_3"] = -a.transform(lambda s: s.shift(2).rolling(3).sum())   # 12h

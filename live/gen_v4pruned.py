"""Generate the targeted-pruned v4 model: drop ret_3d, obv_z_1d, autocorr_pctile_7d, return_1d, resid_rev_3
(dead weight/dilutive per LOO); keep vol features + vwap_slope_96 + resid_rev_2. Residual target. Measure tip; save preds.
"""
...
DROP={"ret_3d","obv_z_1d","autocorr_pctile_7d","return_1d"}
FEATS=[c for c in V0 if c not in DROP]+["resid_rev_2"]
...
print(f"pruned feats ({len(FEATS)}): {FEATS}",flush=True)
rec=[]
    ...
    for sym,gg in tr.groupby("symbol"):
        ...
        try:
            ...
            if len(gte): rec.append(pd.DataFrame({"symbol":sym,"open_time":gte["open_time"].values,
                ...
                "exit_time":gte["exit_time"].values,"pred":m.predict(x6.apply_preproc(gte,FEATS,s,h)),"fold":i,"fwd":gte["fwd"].values}))
...
# tip
d=out[out.open_time>=pd.Timestamp("2025-10-04",tz="UTC")].dropna(subset=["fwd"])
def tip(K):
    ls=[gc.nlargest(K,"pred")["fwd"].mean()-gc.nsmallest(K,"pred")["fwd"].mean() for _,gc in d.groupby("open_time") if len(gc)>=2*K]
    s=pd.Series(ls); return s.mean(),s.mean()/s.std()*np.sqrt(len(s))
for K in [2,3]: m,sh=tip(K); print(f"PRUNED tip K={K}: mean{m:+.1f} Sharpe{sh:+.2f}",flush=True)
o=REPO/"live/state/convexity/hl_v4pruned/v0full_hl60.parquet"; o.parent.mkdir(parents=True,exist_ok=True)
out.drop(columns=["fwd"]).to_parquet(o)
print("PGENDONE")

...
-> live/state/convexity/hl_residrev2/{fullflow_hl60,v0full_hl60}.parquet
"""
...
OUT = REPO/"live/state/convexity/hl_residrev2"; OUT.mkdir(parents=True, exist_ok=True)
RR = ["resid_rev_2","resid_rev_3","resid_rev_6","resid_rev_12","resid_accel"]
...
PAN["resid_rev_6"] = -a.transform(lambda s: s.shift(1).rolling(6).sum())   # 24h
PAN["resid_rev_12"] = -a.transform(lambda s: s.shift(1).rolling(12).sum()) # 48h
PAN["resid_accel"] = PAN["resid_rev_2"] - PAN["resid_rev_6"]/3.0           # reversal acceleration

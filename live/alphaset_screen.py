"""Generalized factor screen vs the beta-neutral residual target (parametrized version of
live/alpha191_screen.py — identical metrics, env-driven paths/prefix).

Two per-factor metrics, per-cycle cross-sectional Spearman aggregated over cycles:
  raw_IC  : factor vs forward 24h residual alpha (alpha_vs_btc_realized, 6-bar fwd sum)
  marg_IC : resid(factor ~ V0_LEAN) vs resid(target ~ V0_LEAN)  [pooled OLS] — orthogonal signal.
...
d=d[d.open_time>=pd.Timestamp("2025-10-04",tz="UTC")]     # in-sample trading window
...
print("\n=== TOP 30 by |marginal-IC t-stat| (orthogonal to V0_LEAN) ===")
print(res.head(30).to_string(index=False,float_format=lambda v:f"{v:+.4f}"))

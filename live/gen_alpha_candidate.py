"""Backtest candidate: add alpha065 (GTJA MA6/close reversal) to V0_LEAN. Generates row-matched baseline
(V0_LEAN) and treatment (V0_LEAN+alpha065) two-book WF preds using the EXACT gen_fundz machinery (same cuts,
HL=60, xs_z target, per-symbol RidgeCV) so only the feature set differs.
  base_lean   : V0_LEAN            -> hl_a065base_lean/v0full_hl60.parquet
  long_lean   : V0_LEAN+RR         -> hl_a065long_lean/v0full_hl60.parquet
  base_a065   : V0_LEAN+alpha065   -> hl_a065base/v0full_hl60.parquet
  long_a065   : V0_LEAN+alpha065+RR-> hl_a065long/v0full_hl60.parquet
"""
...
RR=["resid_rev_2","resid_rev_3"]; import os
ADD=[os.environ["FACTOR"]]
FPATH=os.environ.get("FACTORS_PATH",str(REPO/"data/ml/cache/alpha191_factors.parquet"))
GENBASE=os.environ.get("GEN_BASELINE","0")=="1"
...
# merge alpha065 (only the col), drop rows where it's NaN so baseline & treatment share EXACT rows
fac=pd.read_parquet(FPATH,columns=["symbol","open_time"]+ADD)
fac["open_time"]=pd.to_datetime(fac["open_time"],utc=True)
PAN=PAN.merge(fac,on=["symbol","open_time"],how="left").dropna(subset=ADD)
...
print(f"rows={len(PAN)} (alpha065 merged, non-nan); syms={PAN.symbol.nunique()}",flush=True)
...
D=REPO/"live/state/convexity"; FN=ADD[0]
if GENBASE:
    print("base_lean", gen(V0_LEAN,       D/"hl_cand_base_lean/v0full_hl60.parquet"),flush=True)
    print("long_lean", gen(V0_LEAN+RR,    D/"hl_cand_long_lean/v0full_hl60.parquet"),flush=True)
print(f"base_{FN}", gen(V0_LEAN+ADD,      D/f"hl_cand_{FN}_base/v0full_hl60.parquet"),flush=True)
print(f"long_{FN}", gen(V0_LEAN+ADD+RR,   D/f"hl_cand_{FN}_long/v0full_hl60.parquet"),flush=True)
print("GENDONE")

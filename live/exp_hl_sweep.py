...
x6=tt.x6; V0=list(tt.V0); RR=["resid_rev_2","resid_rev_3"]; EMB=pd.Timedelta(days=1); feats=V0+RR
CUTS=[pd.Timestamp(t,tz="UTC") for t in ["2025-10-04","2025-11-01","2025-12-01","2026-01-01","2026-02-01","2026-03-01","2026-04-01","2026-05-01","2026-05-27","2026-06-30"]]
    ...
    for i in range(len(CUTS)-1):
        ...
        for sym,gg in tr.groupby("symbol"):
            ...
            try:
                ...
                if len(gte): rec.append(pd.DataFrame({"symbol":sym,"open_time":gte["open_time"].values,"pred":m.predict(x6.apply_preproc(gte,feats,s,h)),"xs_z":gte["xs_z"].values}))
            except Exception: pass
    return pd.concat(rec,ignore_index=True)
print("HL sweep — per-cycle IC (full OOS). current HL=60:")
for HL in [20,40,60,90,120]:
    t0=time.time(); W=gen(HL); W["open_time"]=pd.to_datetime(W["open_time"],utc=True)
    ic=W.dropna().groupby("open_time").apply(lambda x:x["pred"].corr(x["xs_z"],method="spearman")).dropna()
    print(f"  HL={HL:3d}d: IC mean {ic.mean():+.4f}  %>0 {(ic>0).mean()*100:.0f}%  ({time.time()-t0:.0f}s)")
print("DONE hl_sweep")

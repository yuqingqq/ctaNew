"""Generate incremental-prune pred sets (residual target, single model both legs) for the fixed-K fair-fee test:
baseline is hl_tgt_res_long (have it). Here we make: drop-ret_3d, drop-obv_z_1d, drop-{both}. Saved with fold col.
"""
...
gen([c for c in BASE if c!="ret_3d"],       "hl_pv_no_ret3d")
gen([c for c in BASE if c!="obv_z_1d"],      "hl_pv_no_obvz")
gen([c for c in BASE if c not in {"ret_3d","obv_z_1d"}], "hl_pv_no_both")
print("PVDONE")

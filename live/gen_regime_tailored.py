"""Generate regime-TAILORED OOS books (residual target, same WF pipeline as gen_oos_v4).
Motivated by the per-regime feature map (deep_v4_feat_regime.py):
  - bear LONG: drop the features that REVERSE in bear-long -> bars_since_high, bars_since_high_xs_rank, ret_3d.
  - bear SHORT: drop the features that REVERSE in bear-short -> corr_to_btc_1d, idio_vol_to_btc_1d, rvol_7d
                (leaves beta_change + the bear-stable set to dominate).
These are used ONLY in bear (regime-conditional) by the comparison script; side/bull keep pooled v4.
Outputs: hl_v4long_bearcal (long, +RR, minus long-reversers) ; hl_v4base_bearcal (base, minus short-reversers).
"""
import sys
...
LONG_DROP = {"bars_since_high", "bars_since_high_xs_rank", "ret_3d"}
SHORT_DROP = {"corr_to_btc_1d", "idio_vol_to_btc_1d", "rvol_7d"}
START = pd.Timestamp("2023-01-01", tz="UTC"); END = pd.Timestamp("2025-10-01", tz="UTC")
CUTS = list(pd.date_range(START, END, freq="MS", tz="UTC"))
print(f"tailored OOS CUTS: {CUTS[0].date()} .. {CUTS[-1].date()} ({len(CUTS)-1} folds)", flush=True)
...
def gen(feats, outpath):
    ...
    for i in range(len(CUTS)-1):
        ...
        print(f"  fold {i} {c0.date()}", flush=True)
...
long_feats = [f for f in V0_LEAN+RR if f not in LONG_DROP]
base_feats = [f for f in V0_LEAN if f not in SHORT_DROP]
print(f"bear-LONG feats ({len(long_feats)}): {long_feats}", flush=True)
print(f"bear-SHORT feats ({len(base_feats)}): {base_feats}", flush=True)
print("=== bear-long book ===", flush=True)
print(gen(long_feats, REPO/"live/state/convexity/hl_v4long_bearcal/v0full_hl60.parquet"), flush=True)
print("=== bear-short book ===", flush=True)
print(gen(base_feats, REPO/"live/state/convexity/hl_v4base_bearcal/v0full_hl60.parquet"), flush=True)
print("TAILOREDDONE")

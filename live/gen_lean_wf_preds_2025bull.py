"""Generate LEAN walk-forward preds covering the FULL 175-sym panel — schema derived from the PANEL, NOT
from a merge against the stale x132 baseline (which capped all prior backtest preds at 160 syms and dropped
the ~8 newly-eligible coins: 0G/2Z/ASTER/AVNT/CC/MET/MON/SKY).

Flow book = V0_lean(14, no funding) + VPIN/TFI(4); price book = V0_lean(14). Monthly walk-forward, recency-60.
Outputs lean 175-covering preds for the honest 175-universe backtest.

Usage: python3 live/gen_lean_wf_preds.py  -> live/state/convexity/hl_lean175/{fullflow_hl60,v0full_hl60}.parquet
"""
import sys
...
CUTS = [pd.Timestamp(t, tz="UTC") for t in ["2025-04-01","2025-05-01","2025-06-01","2025-07-01","2025-08-01","2025-09-01"]]
OUT = REPO/"live/state/convexity/hl_lean175_2025bull"; OUT.mkdir(parents=True, exist_ok=True)
...
def gen(use_flow, outpath):
    ...
    for i in range(len(CUTS)-1):
        ...
        for sym, g in tr.groupby("symbol"):
            ...
            feats = V0_LEAN + fk if uf else V0_LEAN
            try:
...
print(f"2025-bull price preds: {nps} syms/{npr} rows -> {OUT}"); print("DONE_GEN2025")

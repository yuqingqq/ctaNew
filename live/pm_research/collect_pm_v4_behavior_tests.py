...
Drives PMCollector._market against fake websockets. Four paths:
  O1a  connect kwargs carry ping_interval=3, ping_timeout=3
...
async def main():
    ...
    check("O1a: ping_interval=3", kw.get("ping_interval") == 3, str(kw))
    check("O1a: ping_timeout=3", kw.get("ping_timeout") == 3, str(kw))

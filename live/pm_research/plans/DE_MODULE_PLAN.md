...
| contract addition (additive) | `Action.order_ref`; `Action.placement ∈ {JOIN, FRONT_ON_FORMATION}`; `CapitalOpCommand` + `capital_cmd_out/in` ports (§4); `DE-Constraints` consumes `CapitalBudget` (module record — closes the size chain, §4); `telemetry_out` made explicit on the Constraints/Allocator records **AND `HealthEvent` added to their `produces` lists — a port without a produces entry is half a fix** (OP cross-review, iteration 1) — with named `HealthEvent` sources (§3.5); **`HeartbeatPulse` emission for the four acting DE modules** (the OP cross-review found the liveness gap recursing: an event-only module that wedges emits nothing and no pulse goes STALE); **`FeasibleSet.max_size` notes: side-keyed key domain + default-DENY (§2a — without it "∅" has no carrier and an empty set is permissive)**. Also flagged there: the contracts ports-map representation is self-inconsistent (`_representation` says records own ports while OP-Monitor's ports live only in the map) — a contracts-hygiene item for the same batch |

#!/usr/bin/env bash
# v4 gate-2: frozen v3_native strategy + matched execution, vary ONLY the training target of the preds
# (row-matched hl_tgt_ret_* vs hl_tgt_res_* from gen_residual_target.py).
set -uo pipefail
...
run(){ local tag=$1 pb=$2 pl=$3; local sd=live/state/longtail/v4_ab/$tag; rm -rf "$sd"; mkdir -p "$sd"
  ...
  echo "$tag rc=$?"; }
run ret hl_tgt_ret_base hl_tgt_ret_long &
run res hl_tgt_res_base hl_tgt_res_long &
wait; echo V4ABDONE

"""Generate v4+factors model: V0_LEAN + resid_rev + [ΔIC-survivor factors], residual target, single book (both legs).
For the quick decisive NET test vs v4 baseline. Factors from beta-neutral parquet.
"""
...
FACT=["alpha159","alpha082","alpha023","alpha010"]           # de-duplicated ΔIC survivors
...
fac=pd.read_parquet(REPO/"data/ml/cache/alpha191_factors_betaneut.parquet",columns=["symbol","open_time"]+FACT)
fac["open_time"]=pd.to_datetime(fac["open_time"],utc=True)
PAN=PAN.merge(fac,on=["symbol","open_time"],how="left")
for c in FACT: PAN[c]=PAN[c].fillna(0.0)
...
FEATS=V0_LEAN+RR+FACT
print(f"rows={len(PAN)} feats={len(FEATS)} (V0_LEAN{len(V0_LEAN)}+RR2+{len(FACT)}factors)",flush=True)
rec=[]
...
o=REPO/"live/state/convexity/hl_v4fac_long/v0full_hl60.parquet"; o.parent.mkdir(parents=True,exist_ok=True); out.to_parquet(o)
print(f"GENDONE {out['symbol'].nunique()} syms {len(out)} rows")

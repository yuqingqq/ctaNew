...
run(){ local tag=$1 ks=$2; local sd=live/state/longtail/v4_net_k_ins/$tag; rm -rf "$sd"; mkdir -p "$sd"
  env "${EXEC[@]}" "${V3[@]}" STRAT_K=$ks \
      CONVEXITY_PREDS_PATH="$D/hl_tgt_res_base/v0full_hl60.parquet" CONVEXITY_PREDS_LONG="$D/hl_tgt_res_long/v0full_hl60.parquet" \
      CONVEXITY_STATE=$sd PYTHONPATH=. $PY -m live.convexity_paper_bot --replay-all > "$sd/run.log" 2>&1
  echo "$tag(K_short=$ks) rc=$?"; }
run kshort2 2 &
run kshort3 3 &
wait; echo V4NETKINSDONE

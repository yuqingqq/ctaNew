"""LOOP2 iter-08 — HYBRID full-universe preds: flow-syms use V0+flow, non-flow syms use V0.
Per-sym Ridge xs_z + recency60 monthly-WF. Deployment test vs +2.95 full-universe champion.
"""
...
OUT=REPO/"live/state/convexity/hybrid_flow_preds.parquet"
...
def main():
    t0=time.time(); print("=== LOOP2 iter-08: HYBRID full-universe (flow where avail) ===\n",flush=True)
    ...
    pan=pan.merge(F,on=["symbol","open_time"],how="left")  # left: non-flow syms get NaN flowcols
    ...
    for i in range(len(CUTS)-1):
        ...
        for sym,g in tr.groupby("symbol"):
            ...
            # use flow features only if this sym has them populated in training
            use_flow = (sym in flowsyms) and g[flowcols].notna().any().all()
            feats=V0+flowcols if use_flow else V0
            try:
    ...
    n=oos["pred_n"].notna().sum(); oos["pred"]=oos["pred_n"].where(oos["pred_n"].notna(),oos["pred"]); oos.drop(columns=["pred_n"]).to_parquet(OUT)

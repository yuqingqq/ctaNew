...
This script:
  ...
  5. Saves all artifacts to results/step58b_clean108_causal/
"""
...
OUT = REPO / "linear_model/results/step58b_clean108_causal"
...
def main():
    ...
    print("  STEP 58b: clean-108 causal + placebos + per-symbol attribution", flush=True)
    print("=" * 100, flush=True)
    ...
    # ---- Per-symbol attribution: is alpha now diversified or shifted to next outlier? ----
    print(f"\n{'='*100}", flush=True)
    print("  PER-SYMBOL ATTRIBUTION (clean-108)", flush=True)
    print(f"{'='*100}", flush=True)
    recs = psl.run_production_protocol_save_sleeves(apd_v, universe_V2)
    al = apd_full.set_index(["open_time","symbol"])["alpha_beta"].to_dict()
    from collections import defaultdict as _dd
    contrib=_dd(float); npk=_dd(int)
    for _,r in recs.iterrows():
        if not r["traded"]: continue
        tt=r["time"]; L=list(r["long_basket"]); S=list(r["short_basket"])
        nL,nS=len(L),len(S)
        for x in L:
            a=al.get((tt,x))
            if a is not None and not pd.isna(a): contrib[x]+=a*1e4/max(nL,1); npk[x]+=1
        for x in S:
            a=al.get((tt,x))
            if a is not None and not pd.isna(a): contrib[x]+=-a*1e4/max(nS,1); npk[x]+=1
    ad=pd.DataFrame({"symbol":list(contrib),"contrib_bps":[contrib[k] for k in contrib],
                     "n_picks":[npk[k] for k in contrib]}).sort_values("contrib_bps",ascending=False)
    tot=ad.contrib_bps.sum()
    top5=ad.head(5).contrib_bps.sum()
    print(f"  total contrib: {tot:,.0f} bps | top-5 symbols: {top5:,.0f} bps ({top5/tot*100 if tot else 0:.0f}% of total)", flush=True)
    print(ad.head(10).to_string(index=False), flush=True)
    print("  ...", flush=True)
    print(ad.tail(5).to_string(index=False), flush=True)
    ad.to_csv(OUT / "attribution.csv", index=False)

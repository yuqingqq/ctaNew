"""Step 30: R3 + return_8h + vol_zscore_4h_over_7d

Two strongest new features at 4h-aligned horizons (from Step 29 audit):
  - return_8h          (|spearman| 0.039, monotonic, d9-d0 spread = 39 bps)
  - vol_zscore_4h_over_7d  (|sp| 0.022, monotonic, d9-d0 = 11.5 bps)

Add to R3 (22 features) → R3_8h_VZ (24 features). Use corrected pipeline.
Validate with LOFO + P2 placebo.
"""
...
NEW_FEATS = ["return_8h", "vol_zscore_4h_over_7d"]  # both monotonic per Step 29
...
def main():
    print("=== Step 30: R3 + return_8h + vol_zscore_4h_over_7d ===\n", flush=True)
    t0 = time.time()
    ...
    step29 = pd.read_parquet(STEP29, columns=["symbol","open_time"] + NEW_FEATS)
    ...
    print(f"R3_8h_VZ features: {len(feat_cols)} (R3 22 + 2 new)", flush=True)
    ...
    for fid in ALL_FOLDS:
        ...
        for seed in SEEDS:
            ...
            # Track new feature coefs
            for nf in NEW_FEATS:
                nf_idx = feat_cols.index(nf)
                coef_records.append({"fold":fid, "seed":seed, "feature":nf,
                                      "coef":float(m.coef_[nf_idx]),
    ...
    apd.to_parquet(OUT / "ridge_r3_8h_vz_preds.parquet", index=False)
    cyc_ic = apd.dropna(subset=["alpha_beta"]).groupby("open_time").apply(
        lambda g: g["pred_z"].rank().corr(g["alpha_beta"].rank())
        if len(g) >= 5 else np.nan).dropna()
    print(f"\nR3_8h_VZ overall IC: {cyc_ic.mean():+.4f} (vs R3 corrected +0.0134)",
          flush=True)

    # Coef summary
    coef_df = pd.DataFrame(coef_records)
    print(f"\nNew feature coefficients:", flush=True)
    for nf in NEW_FEATS:
        c = coef_df[coef_df["feature"]==nf]["coef"]
        print(f"  {nf:<28} mean={c.mean():+.4f} std={c.std():.4f} "
              f"stab={abs(c.mean())/c.std():.1f}", flush=True)
    ...
    print(f"\n{'='*90}", flush=True)
    print(f"  R3_8h_VZ BACKTEST", flush=True)
    ...
    for label, col in [("A_baseline","pred_z"), ("B_IC_signed","pred_B")]:
        ...
        print(f"  R3_8h_VZ {label}: Sharpe={sh:+.2f} [{lo:+.2f},{hi:+.2f}], "
              f"end-eq=${end_eq:.2f}, gross={df_v['gross_pnl_bps'].mean():+.2f}, "
              f"folds+={folds_positive(df_v)}/9", flush=True)
        df_v.to_csv(OUT / f"r3_8h_vz_backtest_{label}.csv", index=False)
        if label == "B_IC_signed":
            sh_main = sh
            df_v_main = df_v

    # LOFO
    print(f"\n  LOFO on R3_8h_VZ B_IC_signed (Sharpe = {sh_main:+.2f}):", flush=True)
    ...
    # P2 placebo
    print(f"\n--- P2 placebo (R3_8h_VZ univ + random picks, {N_PLACEBO} seeds) ---",
          flush=True)
    p2 = []
    ...
    print(f"  R3_8h_VZ rank in P2: {(p2 < sh_main).mean()*100:.1f}%", flush=True)
    print(f"  Edge over P2 p95: {sh_main - p95:+.2f}", flush=True)

    pd.DataFrame({"placebo":p2}).to_csv(OUT / "r3_8h_vz_placebo.csv", index=False)

    print(f"\nReferences:", flush=True)
    print(f"  R3 corrected + IC-signed:           +0.86 (p82 vs P2, folds 1+2 drive)",
          flush=True)
    print(f"  R3_VS + IC-signed:                  +0.64 (p76, worse)", flush=True)
    print(f"  R3_8h_VZ B (this):                  {sh_main:+.2f}", flush=True)

...
Tests whether V2's +3.11 on 110-panel is robust to symbol composition.
...
110-panel baseline: +3.11 (Step 41 rerun)
"""
...
PANEL_PIT = REPO / "outputs/vBTC_features_btc_only_111_full_pit/panel_btc_only_111.parquet"
...
def aggregate_hold_through(records, alpha_wide):
    ...
    for _, rec in records.iterrows():
        ...
        # CAUSAL: use NEW weights tw for gross PnL
        gross = 0.0
...
def main():
    ...
    print("  STEP 51: V2 110-panel UNIVERSE STRESS TEST", flush=True)
    print("=" * 100, flush=True)
    print(f"  K_drop ∈ {K_DROPS}, {N_DRAWS} random draws per K", flush=True)
    print(f"  Baseline: V2 on 110-panel + V3.1 sleeve = +3.11", flush=True)
    t0 = time.time()
    ...
    for K_drop in K_DROPS:
        ...
        n_pos = int((shs > 0).sum())
        BASELINE_SHARPE = 3.1070703605702614  # Step 50 causal full-PIT
        n_beat_baseline = int((shs > BASELINE_SHARPE).sum())
        ...
        print(f"           {n_pos}/{N_DRAWS} positive, "
              f"{n_beat_baseline}/{N_DRAWS} beat baseline +3.11", flush=True)
    ...
    pd.DataFrame(results).to_csv(OUT / "step51_110_causal_universe_stress.csv", index=False)

...
Tests whether V2's +2.11 on 110-panel is robust to symbol composition.
...
110-panel baseline: +2.11 (Step 41 rerun)
"""
...
PANEL_PIT = REPO / "outputs/vBTC_features_btc_only_111_strict_pit/panel_btc_only_111.parquet"
...
def main():
    ...
    print("  STEP 46: V2 110-panel UNIVERSE STRESS TEST", flush=True)
    print("=" * 100, flush=True)
    print(f"  K_drop ∈ {K_DROPS}, {N_DRAWS} random draws per K", flush=True)
    print(f"  Baseline: V2 on 110-panel + V3.1 sleeve = +2.11", flush=True)
    t0 = time.time()
    ...
    for K_drop in K_DROPS:
        ...
        print(f"           {n_pos}/{N_DRAWS} positive, "
              f"{n_beat_baseline}/{N_DRAWS} beat baseline +2.11", flush=True)
    ...
    pd.DataFrame(results).to_csv(OUT / "step46_110_strict_pit_universe_stress.csv", index=False)

...
Input panel: 111 symbols from outputs/vBTC_features_btc_only_111_full_pit/
...
OUT = REPO / "linear_model/results/step56_hl_native"
...
def main():
    ...
    print("  STEP 56: HL-NATIVE retrain (train+validate on HL-executable subset only)", flush=True)
    print("=" * 100, flush=True)
    t0 = time.time()
    ...
    # === HL-NATIVE restriction ===
    import os
    HL_FILTER = os.environ.get("HL_FILTER", "onhl")  # onhl(70) | vol5m(30)
    hl_map = pd.read_csv(REPO / "outputs/vBTC_check_universe/panel110_hl_map.csv")
    if HL_FILTER == "onhl":
        keep = set(hl_map[hl_map.on_hl]["symbol"])
    elif HL_FILTER == "vol5m":
        keep = set(hl_map[(hl_map.on_hl) & (hl_map.hl_day_vol_usd >= 5e6)]["symbol"])
    else:
        keep = set(panel["symbol"].unique())
    n0 = panel["symbol"].nunique()
    panel = panel[panel["symbol"].isin(keep)].copy()
    print(f"[HL-native filter={HL_FILTER}] kept {panel['symbol'].nunique()}/{n0} symbols",
          flush=True)
    ...
    # Recompute sigma_idio on HL-restricted fold-0 (per-symbol; median fallback now
    # over HL symbols only) so target_z is HL-native, not anchored to the 110 dist.
    ...
    print(f"  Recomputed sigma_idio on HL fold-0 (median {med:.5f})", flush=True)

...
This script:
  ...
  5. Saves all artifacts to results/step57_drop_siren_jelly/
"""
...
OUT = REPO / "linear_model/results/step57_drop_siren_jelly"
...
def main():
    ...
    print("  STEP 57: drop ONLY SIREN+JELLYJELLY (keep other 108) — is the edge just these 2?", flush=True)
    print("=" * 100, flush=True)
    ...
    n0 = apd_full["symbol"].nunique()
    apd_full = apd_full[~apd_full["symbol"].isin(DROP)].copy()
    print(f"\n[drop-2] removed {DROP}: {apd_full['symbol'].nunique()}/{n0} symbols kept", flush=True)
    panel_syms = sorted(apd_full["symbol"].unique())
    print(f"\nPanel symbols: {len(panel_syms)} (108, SIREN+JELLYJELLY dropped)", flush=True)

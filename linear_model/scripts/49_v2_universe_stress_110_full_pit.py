...
Tests whether V2's +3.35 on 110-panel is robust to symbol composition.
...
110-panel baseline: +3.35 (Step 41 rerun)
"""
...
def main():
    ...
    print("  STEP 49: V2 110-panel UNIVERSE STRESS TEST", flush=True)
    print("=" * 100, flush=True)
    print(f"  K_drop ∈ {K_DROPS}, {N_DRAWS} random draws per K", flush=True)
    print(f"  Baseline: V2 on 110-panel + V3.1 sleeve = +3.35", flush=True)
    t0 = time.time()
    ...
    for K_drop in K_DROPS:
        ...
        n_pos = int((shs > 0).sum())
        n_beat_baseline = int((shs > 3.35).sum())  # baseline is full-PIT +3.35
        ...
        print(f"           {n_pos}/{N_DRAWS} positive, "
              f"{n_beat_baseline}/{N_DRAWS} beat baseline +3.35", flush=True)
    ...
    pd.DataFrame(results).to_csv(OUT / "step49_110_full_pit_universe_stress.csv", index=False)

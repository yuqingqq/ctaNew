...
This script:
  ...
  5. Saves all artifacts to results/step55_hl_transport/
"""
...
OUT = REPO / "linear_model/results/step55_hl_transport"
...
def main():
    ...
    print("  STEP 55: HL-executable universe transport (restrict to HL-tradeable symbols)", flush=True)
    print("=" * 100, flush=True)
    ...
    import os
    HL_FILTER = os.environ.get("HL_FILTER", "onhl")  # onhl | vol1m | vol5m
    hl_map = pd.read_csv(REPO / "outputs/vBTC_check_universe/panel110_hl_map.csv")
    if HL_FILTER == "onhl":
        keep = set(hl_map[hl_map.on_hl]["symbol"])
    elif HL_FILTER == "vol1m":
        keep = set(hl_map[(hl_map.on_hl) & (hl_map.hl_day_vol_usd >= 1e6)]["symbol"])
    elif HL_FILTER == "vol5m":
        keep = set(hl_map[(hl_map.on_hl) & (hl_map.hl_day_vol_usd >= 5e6)]["symbol"])
    else:
        keep = set(apd_full["symbol"].unique())
    n_before = apd_full["symbol"].nunique()
    apd_full = apd_full[apd_full["symbol"].isin(keep)].copy()
    print(f"\n[HL transport] filter={HL_FILTER}: {apd_full['symbol'].nunique()}/{n_before} symbols kept", flush=True)
    panel_syms = sorted(apd_full["symbol"].unique())
    print(f"\nPanel symbols: {len(panel_syms)} (HL-executable subset)", flush=True)

...
OUT = REPO / "linear_model/results/step58_clean108"
...
FRAME_NEUTRAL = ["return_1d","atr_pct","obv_z_1d","vwap_slope_96",
                 "bars_since_high_xs_rank","bars_since_low_xs_rank",
...
def main():
    ...
    print("  STEP 58: CLEAN-108 retrain (drop ONLY SIREN+JELLYJELLY, full retrain on 108)", flush=True)
    print("=" * 100, flush=True)
    t0 = time.time()
    ...
    # === CLEAN-108: drop ONLY the two outliers, no HL restriction ===
    DROP = {"SIRENUSDT", "JELLYJELLYUSDT"}
    n0 = panel["symbol"].nunique()
    panel = panel[~panel["symbol"].isin(DROP)].copy()
    print(f"[clean-108] dropped {DROP}: kept {panel['symbol'].nunique()}/{n0} symbols",
          flush=True)

"""Step 27: R3 + vol_surge_1d_over_30d — add the strongest volume feature.

R3_VS = R3 (22 features) + vol_surge_1d_over_30d (monotonic, |pearson| 0.047)
     = 23 features

Run through CORRECTED pipeline (PIT σ fallback + hold-through aggregator)
plus LOFO + placebo to verify it's not just another fold-concentrated win.
"""
...
NEW_VOL = ["vol_surge_1d_over_30d"]  # monotonic, |pearson| 0.047
...
def main():
    print("=== Step 27: R3 + vol_surge_1d_over_30d ===\n", flush=True)
    t0 = time.time()
    ...
    vol = pd.read_parquet(VOL_SURGE, columns=["symbol","open_time"] + NEW_VOL)
    vol["open_time"] = pd.to_datetime(vol["open_time"], utc=True)
    panel = tgt.merge(base, on=["symbol","open_time"], how="left")
    panel = panel.merge(vol, on=["symbol","open_time"], how="left")
    ...
    print(f"R3_VS features: {len(feat_cols)} (R3 22 + 1 vol_surge)", flush=True)

    # Train
    print("\nTraining R3_VS...", flush=True)
    all_preds = []
    coef_records = []
    for fid in ALL_FOLDS:
        ...
        for seed in SEEDS:
            ...
            # Record vol_surge coefficient specifically
            vs_idx = feat_cols.index("vol_surge_1d_over_30d")
            coef_records.append({"fold":fid, "seed":seed,
                                  "vol_surge_coef":float(m.coef_[vs_idx]),
    ...
    apd.to_parquet(OUT / "ridge_r3_vs_preds.parquet", index=False)
    cyc_ic = apd.dropna(subset=["alpha_beta"]).groupby("open_time").apply(
        lambda g: g["pred_z"].rank().corr(g["alpha_beta"].rank())
        if len(g) >= 5 else np.nan).dropna()
    print(f"\nR3_VS overall IC: {cyc_ic.mean():+.4f}", flush=True)

    # Vol surge coefficient summary
    coef_df = pd.DataFrame(coef_records)
    print(f"\nvol_surge_1d_over_30d coefficient:", flush=True)
    print(f"  mean: {coef_df['vol_surge_coef'].mean():+.4f}", flush=True)
    print(f"  std:  {coef_df['vol_surge_coef'].std():.4f}", flush=True)
    print(f"  stability: {abs(coef_df['vol_surge_coef'].mean())/coef_df['vol_surge_coef'].std():.1f}",
          flush=True)
    ...
    print(f"\n{'='*90}", flush=True)
    print(f"  R3_VS BACKTEST + LOFO", flush=True)
    ...
    for label, col in [("A_baseline","pred_z"), ("B_IC_signed","pred_B")]:
        ...
        print(f"  R3_VS {label}: Sharpe={sh:+.2f} [{lo:+.2f},{hi:+.2f}], "
              f"end-eq=${end_eq:.2f}, gross={df_v['gross_pnl_bps'].mean():+.2f}, "
              f"folds+={folds_positive(df_v)}/9", flush=True)
        df_v.to_csv(OUT / f"r3_vs_backtest_{label}.csv", index=False)
        if label == "B_IC_signed":
            sh_main = sh
            df_v_main = df_v

    # LOFO
    print(f"\n  LOFO on R3_VS B_IC_signed (Sharpe = {sh_main:+.2f}):", flush=True)
    print(f"    {'exclude':>9}  {'rem Sharpe':>11}  {'Δ':>7}  {'fold pnl':>10}",
          flush=True)
    ...
    # P2 placebo
    print(f"\n--- P2 placebo (R3_VS univ + random picks, {N_PLACEBO} seeds) ---",
          flush=True)
    p2 = []
    ...
    print(f"  R3_VS rank in P2: {(p2 < sh_main).mean()*100:.1f}%", flush=True)
    print(f"  Edge over P2 p95: {sh_main - p95:+.2f}", flush=True)

    pd.DataFrame({"placebo":p2}).to_csv(OUT / "r3_vs_placebo.csv", index=False)

    print(f"\nReferences:", flush=True)
    print(f"  R3 corrected + IC-signed:           Sharpe +0.86 (p82 vs P2)", flush=True)
    print(f"  R3_VS B_IC_signed (this run):       Sharpe {sh_main:+.2f}", flush=True)
    print(f"  LGBM production:                    Sharpe +0.74", flush=True)

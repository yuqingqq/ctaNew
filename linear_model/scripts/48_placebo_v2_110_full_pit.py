...
Tests whether V2's Sharpe +3.35 on the 110-panel beats matched-universe
...
References:
  ...
  V2 110-panel + sleeve (Step 41 rerun): Sharpe +3.35, P1/P2 PENDING (this script)
"""
...
PANEL_111_PIT = REPO / "outputs/vBTC_features_btc_only_111_full_pit/panel_btc_only_111.parquet"
...
def run_placebos(apd_v, sampled_t, universe_model, universe_liq, alpha_wide,
    ...
    pd.DataFrame({"P1":p1, "P2":p2}).to_csv(OUT / "step48_placebos_full_pit.csv",
                                              index=False)
...
def main():
    ...
    print("  STEP 48: P1+P2 placebos on V2 110-panel (Step 41 rerun)", flush=True)
    print("=" * 100, flush=True)
    print("  Reference: V2 on 110-panel + sleeve = Sharpe +3.35 (CI [-0.22, +4.14])",
          flush=True)
    ...
    pd.DataFrame([verdict]).to_csv(OUT / "step48_verdict.csv", index=False)

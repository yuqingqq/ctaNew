"""X108 — Confidence-gated ic90 sign-flip on the FULL hybrid, with nested-OOS.

X107: ic90 per-sym sign-flip recovers 2026 (+1.81 PIT) but flips uniformly (hurts good
years). Refinement: flip a sideways pick only on CONFIDENT inversion (trailing ic90 < -tau),
else keep. Restore the regime hybrid this time (mom-bull / mean-rev-side BN / flat-bear, K=5,
the X98 base = +1.84). Validate tau choice with nested-OOS (6-mo blocks; pick tau incl.
'no-flip' from past blocks, apply forward) — the test the throttle FAILED.

Variants: base(no flip) + flip tau in {0, 0.01, 0.02, 0.05}. Report per-year, full, maxDD,
and NESTED-OOS. PASS only if nested >= base AND 2026 rescued.
"""
...
def main():
    t0=time.time()
    print("=== X108 confidence-gated ic90 flip + nested-OOS (full hybrid) ===\n", flush=True)
    ...
    # ic90 per-sym (PIT lag HOLD)
    d["ic90"]=d.groupby("symbol",group_keys=False).apply(
        lambda g: g["pred"].rolling(90,min_periods=45).corr(g["alpha_A"]).shift(HOLD))
    # mom + beta from klines
    ...
    print(f"{len(syms)} syms, {len(times)} cycles\n", flush=True)

    def build(tau):
        """tau=None → no flip. Else flip sideways pick i if ic90_i < -tau."""
        ws=[]
        for ot in times:
            ...
            if rg=="side" and tau is not None:
                fs=np.where(gg["ic90"].fillna(0.0)<-tau,-1.0,1.0)
                gg["score"]=gg["pred"]*fs
                sortcol="score"
            else:
                sortcol=key
            if len(gg)<2*K: ws.append({}); continue
            gg=gg.sort_values(sortcol); L=gg.tail(K)["symbol"].tolist(); S=gg.head(K)["symbol"].tolist()
            a=b=1.0
            if rg=="side":
    ...
    cand={"base":build(None), "flip_t0":build(0.0), "flip_t.01":build(0.01),
          "flip_t.02":build(0.02), "flip_t.05":build(0.05)}
    def row(name,p):
        ...
        print(f"  {name:<12}{ann(p):>+7.2f}{dd:>+9.0f}   {s}", flush=True)
    print(f"  {'variant':<12}{'Sharpe':>7}{'maxDD':>9}   per-year Sharpe")
    for k,p in cand.items(): row(k,p)

    # nested-OOS over 6-mo blocks
    M=pd.DataFrame(cand); names=list(cand)
    blk=(M.index.year-2023)*2+(M.index.month>6).astype(int); blk=pd.Series(blk,index=M.index)
    nested=pd.Series(0.0,index=M.index); picks=[]
    for b in sorted(blk.unique()):
        mask=blk==b; prior=blk<b
        sel="base" if prior.sum()<200 else max(names,key=lambda c: ann(M.loc[prior,c]) if np.isfinite(ann(M.loc[prior,c])) else -9)
        nested.loc[mask]=M.loc[mask,sel].values; picks.append((b,sel))
    print(); row("NESTED-OOS", nested)
    print("  block picks:", " ".join(f"{2023+b//2}{'H1' if b%2==0 else 'H2'}:{s}" for b,s in picks))
    print(f"\nPASS iff nested >= base ({ann(cand['base']):+.2f}) AND 2026 rescued. Done [{time.time()-t0:.0f}s]")

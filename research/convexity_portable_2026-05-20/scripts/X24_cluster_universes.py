"""X24 — Category/cluster-based universe tests using clusters_v1.json.

7 hand-crafted sectors in 51-panel:
  major (4), l1_established (7), l1_newer (11), defi (11), memes (6),
  ai (3), other_alt (9).

Tests (in canonical framework, Ridge Per-sym +cohort):

Single clusters (only ones large enough for K=3 basket):
  C_l1_newer (11), C_defi (11), C_other_alt (9, marginal)

Pair clusters (decent diversification):
  C_L1_all (l1_established + l1_newer = 18)
  C_defi_ai (14)
  C_majors_L1 (22)
  C_majors_defi (15)

Exclusions:
  EX_no_memes (~44)
  EX_no_ai (~47)
  EX_no_memes_ai (~41)
  EX_no_other_alt (~41)
  EX_only_alts (no major, ~46)

Compares to HL-50 baseline +2.01.
"""
from __future__ import annotations
import csv, json, sys, time, warnings, importlib.util, gc, resource
from pathlib import Path
import pandas as pd, numpy as np

warnings.filterwarnings("ignore")
REPO = Path("/home/yuqing/ctaNew")
sys.path.insert(0, str(REPO))
sys.path.insert(0, str(REPO / "scripts"))

OUT = REPO / "research/convexity_portable_2026-05-20/results"
CACHE = OUT / "_cache"

spec = importlib.util.spec_from_file_location("x6",
    REPO / "research/convexity_portable_2026-05-20/scripts/X6_controlled_matrix.py")
x6 = importlib.util.module_from_spec(spec); spec.loader.exec_module(x6)
spec_b = importlib.util.spec_from_file_location("x6b",
    REPO / "research/convexity_portable_2026-05-20/scripts/X6b_cohort_fill.py")
x6b = importlib.util.module_from_spec(spec_b); spec_b.loader.exec_module(x6b)


def log_mem(label=""):
    rss_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024
    print(f"  [MEM {label}] peak_rss={rss_mb:.0f}MB", flush=True)


def get_panel(syms):
    needed = ["symbol", "open_time", "exit_time", "alpha_vs_btc_realized", "return_pct"] + x6.BASE
    panel = pd.read_parquet(REPO / "outputs/vBTC_features/panel_variants_with_funding.parquet",
                            columns=list(set(needed)))
    panel["open_time"] = pd.to_datetime(panel["open_time"], utc=True)
    panel["exit_time"] = pd.to_datetime(panel["exit_time"], utc=True)
    panel = panel[panel["symbol"].isin(syms) & (panel["symbol"] != "BTCUSDT")].copy()
    panel = x6b.build_cohort_fixed(panel)
    panel = x6.build_target_z(panel)
    x6.HEAVY_TAIL.discard("rvol_7d"); x6.HEAVY_TAIL.discard("ret_3d"); x6.HEAVY_TAIL.discard("btc_rvol_7d")
    return panel


def main():
    t0 = time.time()
    print("=== X24 cluster-based universe tests (canonical framework) ===\n", flush=True)
    log_mem("start")

    # Load clusters
    with open(REPO / "config/clusters_v1.json") as f:
        clusters = json.load(f)
    # Drop BTC from any cluster (we exclude BTC from HL-50 anyway)
    clusters_no_btc = {k: [s for s in v if s != "BTCUSDT"] for k, v in clusters.items()}

    # 51-panel syms (excluding BTC)
    panel_syms = set(pd.read_parquet(REPO / "outputs/vBTC_features/panel_variants_with_funding.parquet",
                                      columns=["symbol"])["symbol"].unique()) - {"BTCUSDT"}

    # Intersect each cluster with panel
    panel_clusters = {k: [s for s in v if s in panel_syms] for k, v in clusters_no_btc.items()}

    print("=== Cluster sizes (in 51-panel, no BTC) ===")
    for k, v in panel_clusters.items():
        print(f"  {k}: {len(v)} syms")

    HL_50 = sorted(panel_syms)

    # Build universe variants
    universes = {}

    # Single clusters (only ones >= 6 syms for K=3 long+short basket)
    for k, syms in panel_clusters.items():
        if len(syms) >= 6:
            universes[f"C_{k}"] = sorted(syms)

    # Pair clusters
    L1 = panel_clusters["l1_established"] + panel_clusters["l1_newer"]
    universes["P_L1_all"] = sorted(L1)
    universes["P_defi_ai"] = sorted(panel_clusters["defi"] + panel_clusters["ai"])
    universes["P_majors_L1"] = sorted(panel_clusters["major"] + L1)
    universes["P_majors_defi"] = sorted(panel_clusters["major"] + panel_clusters["defi"])
    universes["P_majors_L1_defi"] = sorted(panel_clusters["major"] + L1 + panel_clusters["defi"])

    # Exclusions
    universes["EX_no_memes"] = sorted(set(HL_50) - set(panel_clusters["memes"]))
    universes["EX_no_ai"] = sorted(set(HL_50) - set(panel_clusters["ai"]))
    universes["EX_no_memes_ai"] = sorted(set(HL_50) - set(panel_clusters["memes"]) - set(panel_clusters["ai"]))
    universes["EX_no_other_alt"] = sorted(set(HL_50) - set(panel_clusters["other_alt"]))
    universes["EX_no_majors"] = sorted(set(HL_50) - set(panel_clusters["major"]))

    print(f"\n=== {len(universes)} cluster-based universes ===")
    for k, v in universes.items():
        print(f"  {k}: {len(v):>3} syms — {v[:5]}...")

    feats = x6.BASE + x6.COHORT_EXTRAS
    results = []
    for u_name, u_syms in universes.items():
        tf = time.time()
        log_mem(f"before {u_name}")
        print(f"\n[{u_name}] {len(u_syms)} syms", flush=True)
        try:
            panel = get_panel(set(u_syms))
            folds = x6.get_folds(panel)
            if len(folds) == 0:
                print(f"  no folds, skipping"); continue
            apd = x6.train_per_sym_ridge(panel, folds, feats, label=u_name)
            pred_path = CACHE / f"x24_{u_name}_preds.parquet"
            apd.to_parquet(pred_path, index=False)
            ic = float(apd["pred"].corr(apd["alpha_A"]))
            print(f"  trained: {len(apd):,} rows, IC={ic:+.4f} [{time.time()-tf:.0f}s]", flush=True)
        except Exception as e:
            print(f"  TRAIN ERR: {e}")
            results.append({"universe": u_name, "n_syms": len(u_syms), "error": str(e)})
            continue
        m = x6.run_sleeve_on_preds(pred_path, f"x24_{u_name}")
        row = {"universe": u_name, "n_syms": len(u_syms), "train_ic": round(ic, 4), **m}
        results.append(row)
        if "sharpe" in m:
            print(f"  sleeve: Sharpe {m['sharpe']:+.2f} folds {m.get('folds_pos','?')} "
                  f"conc {m.get('concentration','?')} PnL {m.get('totPnL','?')}", flush=True)
        del panel, apd; gc.collect()

    keys = ["universe", "n_syms", "train_ic", "sharpe", "ci_lo", "ci_hi",
            "totPnL", "maxDD", "folds_pos", "concentration", "error"]
    out_csv = OUT / "X24_cluster_universes.csv"
    with open(out_csv, "w", newline="") as fp:
        w = csv.DictWriter(fp, fieldnames=keys, extrasaction="ignore")
        w.writeheader()
        for r in results: w.writerow(r)
    print(f"\nSaved {len(results)} → {out_csv} [{time.time()-t0:.0f}s]")
    print(f"\nReference: HL-50 canonical = +2.01")
    for r in results:
        if "sharpe" in r:
            print(f"  {r['universe']:<24} ({r['n_syms']:>3}) Sharpe={r['sharpe']:+.2f} "
                  f"folds={r.get('folds_pos','?')} conc={r.get('concentration','?')}")


if __name__ == "__main__":
    main()

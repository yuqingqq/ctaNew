...
# INCUMBENT_REWEIGHTED_ONLY: PM+FN ONLY (no state features), own artifact.
# Its design matrix is a DIFFERENT width, so it is built separately rather
# than sliced -- slicing would silently assume the state block sits last.
lind = json.loads((FIT / f"linear_d_{COIN}.json").read_text())
mud = np.asarray(lind["norm_mu"], dtype=np.float64)
sdd = np.asarray(lind["norm_sd"], dtype=np.float64)
Xd = np.empty((len(kept), len(mud) + 1), dtype=np.float64)
Xd[:, 0] = 1.0
for j in range(len(kept)):
    Xd[j, 1:] = (np.asarray(PMd[j] + FNd[j], dtype=np.float64) - mud) / sdd
Wd = np.asarray(lind["hazard_weights"], dtype=np.float64)
WMd = np.asarray(lind["value_weights"], dtype=np.float64)
...
scores["INCUMBENT_REWEIGHTED_ONLY"] = logistic(Xd @ Wd) * (Xd @ WMd)
...
THR = {"PLUS_PRED_STATE_V1": lin["causal_thresholds"],
       "LGBM_PINNED": json.loads((FIT / f"lgbm_thresholds_{COIN}.json").read_text()),
       "INCUMBENT_REWEIGHTED_ONLY": lind["causal_thresholds"]}
